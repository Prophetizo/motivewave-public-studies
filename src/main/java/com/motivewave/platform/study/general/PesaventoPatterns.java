...
public class PesaventoPatterns extends Study 
{
  ...
  protected void calculateValues(DataContext ctx)
  {
    ...
    double reversal = getSettings().getDouble(REVERSAL)/100.0;
    ...
    boolean showOlderPivots = getSettings().getBoolean(PivotSet.SHOW_ALL_PIVOTS);
    boolean extendRight = getSettings().getBoolean(PivotSet.EXTEND_RIGHT);
    String pivotType = getSettings().getString(PivotSet.PIVOT_TYPE);
    int start = 1;
    if (!showOlderPivots) {
