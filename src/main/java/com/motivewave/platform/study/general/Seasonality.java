...
public class Seasonality extends com.motivewave.platform.sdk.study.Study 
{
  ...
  protected void calculateValues(DataContext ctx) {
    ...
    if (!bs.isLinear() || bs.getIntervalMinutes() != 1440 || calcInProgress) return;
    calcInProgress = true;
    Util.schedule(() -> {
      try {
        calculate(ctx);
      }
      finally { calcInProgress = false; }
    });
  }
  ...
  private void calculate(DataContext ctx) {
    ...
    boolean soy = Util.compare(getSettings().getString(CALC_METHOD), START_OF_YEAR);
