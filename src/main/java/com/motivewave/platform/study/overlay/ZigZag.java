  ...
  protected void calculateValues(DataContext ctx)
  {
    ...
    if (prev != null && !last.equals(prev.coord)) {
      ...
      for(int i = prev.ind+1; i <= pivotBar; i++) {
        // Hack: we need to trigger an event to fire if this is the Zig Zag Oscillator
        if (!isOverlay() && i == pivotBar) ((DataSeriesImpl)series).setCalcLatest(true);
        series.setFloat(i, Values.DELTA, up ? series.getLow(i) - series.getHigh(prev.ind) : series.getHigh(i) - series.getLow(prev.ind));
        if (!isOverlay() && i == pivotBar) ((DataSeriesImpl)series).setCalcLatest(false);
      }
