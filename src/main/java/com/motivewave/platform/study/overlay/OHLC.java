...
import com.motivewave.platform.databean.DOMNote;
...
import com.motivewave.platform.ui.draw.component.study.DataContextImpl;
...
public class OHLC extends com.motivewave.platform.sdk.study.Study 
{
  ...
  final static String FONT="font", ALIGN="align", OPEN="open", HIGH="high", DHIGH="dhigh", LOW="low", DLOW="dlow", CLOSE="close", 
      ...
      SHORTEN_LATEST="sl", TIMEFRAME="tf", SHOW_IN_DOM="showInDOM";
  ...
  public void initialize(Defaults defaults) {
    ...
    grp.addRow(new BooleanDescriptor(SHOW_IN_DOM, get("LBL_SHOW_IN_DOM"), false, false));
  ...
  protected void calculateValues(DataContext ctx) {
    if (ctx instanceof DataContextImpl) dataCtx = (DataContextImpl)ctx;
    else dataCtx = null;
    
    if (dataCtx != null) studyId = "OHLC:" + dataCtx.getStudyId();

    var series = ctx.getDataSeries();
    var bs = ctx.getDataSeries().getBarSize();
    timeframe = getSettings().getString(TIMEFRAME);
    ...
    try {
      ...
      int maxPrints = s.getInteger(MAX_PRINTS);
      boolean showAll = s.getBoolean(SHOW_ALL);
      shortenLatest = s.getBoolean(SHORTEN_LATEST);
      showInDOM = s.is(SHOW_IN_DOM);
      font = s.getFont(FONT);
      ...
      align = s.getString(ALIGN);
      var r = s.getInteger(RANGE, 5);
      if (Util.compare(s.getString(RANGE_INT), RANGE_MIN)) r *= Util.MILLIS_IN_MINUTE;
      else r *= 1000;
      range = r;
      String rthData = s.getString(RTH);
      ...
      if (ls != null && ls.eod <= ctx.getCurrentTime()) {
        ...
        ls = new LineSet(ls, sod, getEndOfPeriod(sod, instr, false), getStartOfPeriodRTH(sod, instr), getEndOfPeriod(sod, instr, true));
        lines.add(ls);
      }
      else if (ls == null){
        ...
        ls = new LineSet(null, sod, getEndOfPeriod(sod, instr, false), getStartOfPeriodRTH(sod, instr), getEndOfPeriod(sod, instr, true));
        lines.add(ls);
      }
  ...
  public void onTick(DataContext ctx, Tick tick) {
    ...
    if (time >= ls.eod) {
      ...
      long sod = getStartOfNextPeriod(ls.sod, instr, false);
      var nextLs = new LineSet(ls, sod, getEndOfPeriod(sod, instr, false), getStartOfPeriodRTH(sod, instr), getEndOfPeriod(sod, instr, true));
      ...
      if (p < ls.low) { ls.low = p; updated = true; }
      if (p > ls.high) { ls.high = p; updated = true; }
      if (time < ls.sodRth) {
        if (p < ls.olow) { ls.olow = p; updated = true; }
        if (p > ls.ohigh) { ls.ohigh = p; updated = true; }
      }
      if (time >= ls.sodRth && time < ls.sodRth + range) {
        if (p < ls.rLow) { ls.rLow = p; updated = true; }
        if (p > ls.rHigh) { ls.rHigh = p; updated = true; }
      }
      ls.mid = (ls.low + ls.high)/2;
      ls.close = p;
      if (ls.open == Float.MIN_VALUE) { ls.open = p; updated = true; }
      if (updated) updateDOMNotes();
    }
  ...
  private void clearDOMNotes() {
    if (dataCtx == null || dataCtx.getGraph() == null) return;
    var cg = dataCtx.getGraph().getCg();
    var map = cg.getDOMNotes();
    if (Util.isEmpty(map)) return;
    
    try {
      for(var p : new ArrayList<>(map.keySet())) {
        var nlist = map.get(p);
        if (!Util.isEmpty(nlist)) {
          for(var n : new ArrayList<>(nlist)) {
            if (Util.compare(n.getStudyId(), studyId)) nlist.remove(n); 
          }
        }
      }
    }
  ...
  private void updateDOMNotes() {
    ...
    if (openPath.isEnabled()) cg.addDOMNote(ls.open, new DOMNote("OPEN", openPath.getColor(), studyId));
    if (highPath.isEnabled() || showDHigh()) cg.addDOMNote(ls.high, new DOMNote("HIGH", highPath.getColor(), studyId));
    if (lowPath.isEnabled() || showDLow()) cg.addDOMNote(ls.low, new DOMNote("LOW", lowPath.getColor(), studyId));
    if (closePath.isEnabled() && ls.prevClose != 0) cg.addDOMNote(ls.prevClose, new DOMNote("PREV CLOSE", closePath.getColor(), studyId));
    if (pOpenPath.isEnabled() && ls.popen != 0) cg.addDOMNote(ls.popen, new DOMNote("OPEN", pOpenPath.getColor(), studyId));
    if (pLowPath.isEnabled() && ls.plow != 0) cg.addDOMNote(ls.plow, new DOMNote("PREV LOW", pLowPath.getColor(), studyId));
    if (pHighPath.isEnabled() && ls.phigh != 0) cg.addDOMNote(ls.phigh, new DOMNote("PREV HIGH", pHighPath.getColor(), studyId));
    if (showOLow() && ls.olow != 0) cg.addDOMNote(ls.olow, new DOMNote("OVERNIGHT LOW", oLowPath.getColor(), studyId));
    if (showOHigh() && ls.ohigh != 0) cg.addDOMNote(ls.ohigh, new DOMNote("OVERNIGHT HIGH", oHighPath.getColor(), studyId));
    if (midPath.isEnabled() || showDMid()) cg.addDOMNote(ls.mid, new DOMNote("MID", midPath.getColor(), studyId));
    if (rLowPath.isEnabled() && ls.rLow != Float.MAX_VALUE) cg.addDOMNote(ls.rLow, new DOMNote("R LOW", rLowPath.getColor(), studyId));
    if (rHighPath.isEnabled() && ls.rHigh != Float.MIN_VALUE) cg.addDOMNote(ls.rHigh, new DOMNote("R HIGH", rHighPath.getColor(), studyId));
  ...
  private long range;
  private boolean calcInProgress=false, rth, shortenLatest=false, showInDOM=false;
  private Lines figure = new Lines();
  private DataContextImpl dataCtx;
  private String studyId = "OHLC";
  ...
  {
    LineSet(LineSet prev, long sod, long eod, long sodRth, long eodRth) {
  ...
  class BarCalculator implements BarOperation
  {
    ...
    public void onBar(Bar bar) {
      ...
      if (barStart >= ls.eod) { // roll over to the next day
        ...
        long sod = getStartOfNextPeriod(ls.sod, instr, false);
        var nextLs = new LineSet(ls, sod, getEndOfPeriod(sod, instr, false), getStartOfPeriodRTH(sod, instr), getEndOfPeriod(sod, instr, true));
        if (!rth || (barStart >= nextLs.sodRth && barStart < nextLs.eodRth)) {
          nextLs.low = barLow;
          nextLs.high = barHigh;
          nextLs.mid = (nextLs.low + nextLs.high)/2;
        ...
        if (!rth || (barStart >= ls.sodRth && barStart < ls.eodRth)) {
  ...
  class TickCalculator implements TickOperation
  {
    ...
    public void onTick(Tick tick) {
      ...
      if (time >= ls.eod) { // roll over to the next day
        ...
        long sod = getStartOfNextPeriod(ls.sod, instr, false);
        var nextLs = new LineSet(ls, sod, getEndOfPeriod(sod, instr, false), getStartOfPeriodRTH(sod, instr), getEndOfPeriod(sod, instr, true));
        if (!rth || (time >= nextLs.sodRth && time < nextLs.eodRth)) {
        ...
        if (!rth || (time >= ls.sodRth && time < ls.eodRth)) {
